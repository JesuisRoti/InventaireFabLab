...
def show_form(request):
    ...
    # Formulaire standard a deux champs
    form = ConfForm(auto_id=True)

    # Creation du formset avec une seule iteration : extra=1
    sorteForm = formset_factory(SorteForm, extra=1)

    # Recuperation du formulaire gere par le mecanisme formset
    formset = sorteForm()

    # Affichage du template
    return render(request, 'project/my_form.html', {'form': form, 'sorteForm': formset},)
...
def save_form(self):
    ...
    if request.method == 'POST':
        ...
        # Verifie si les donnees du formulaire simple sont valides
        if form.is_valid():
            # Verifie le(s) formulaire(s) du formset
            if formset.is_valid():

                # Affiche les donnees du formulaire simple
                print (form.cleaned_data)

                # Iteration du formset pour afficher les donnees
                for fs in formset:
